...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    try {
      const response = await fetch('http://192.168.1.142:8082/api/auth/register', {
        method: 'POST',
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput style={styles.input} placeholder="Họ và tên" value={fullName} onChangeText={setFullName} placeholderTextColor="#666" />
      ...
      <TouchableOpacity onPress={() => router.push('/(auth)/login')}>
        <Text style={{color: COLORS.primary, marginTop: 20}}>Đã có tài khoản? Đăng nhập ngay</Text>
      </TouchableOpacity>
    </View>
  );
}
